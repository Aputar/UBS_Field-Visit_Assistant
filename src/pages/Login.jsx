...
export default function Login({ onLogin, loginFn, users, offline }) {
  ...
  const handleLogin = async () => {
    if (!name.trim() || !password.trim()) return setError("Please enter your name and password.");
    setLoading(true);
    setError("");
    try {
      let user = null;
      if (loginFn) {
        // Supabase login
        user = await loginFn(name, password);
      } else {
        // Offline fallback
        user = (users || []).find(u =>
      ...
      else { setError("Invalid name or password. Please check and try again."); }
    } catch (e) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", background: "#ffffff", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 32, textAlign: "center" }}>
        ...
        {offline && <div style={{ marginTop: 8, fontSize: 11, color: "#c47a00", background: "#fff8e8", padding: "4px 12px", borderRadius: 99, display: "inline-block" }}>⚠ Offline mode</div>}
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 360, border: "1px solid #e2e5ea", boxShadow: "0 4px 24px rgba(15,39,68,0.10)" }}>
        ...
        {error && (
          <div style={{ color: "#d63230", fontSize: 12, marginBottom: 12, padding: "8px 10px", background: "#fdf0f0", borderRadius: 6 }}>{error}</div>
        )}
        ...
        <div style={{ marginTop: 16, padding: "12px 14px", background: "#f4f5f7", borderRadius: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#4b5563", marginBottom: 6 }}>DEMO LOGINS — tap to fill</div>
          {[
            ...
            <div key={u.name} onClick={() => quickFill(u.name, u.pass)}
              style={{ fontSize: 12, color: "#0f2744", padding: "5px 0", cursor: "pointer", borderBottom: "1px solid #e2e5ea", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>{u.name}</span>
              <span style={{ color: "#9ca3af", fontSize: 11 }}>{u.role} · {u.pass}</span>
            </div>
          ))}
          <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 6 }}>These work after Supabase is set up</div>
        </div>
      </div>
    </div>
  );
